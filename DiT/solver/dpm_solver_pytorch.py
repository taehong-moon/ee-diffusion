    ...
    def model_fn(x, t_continuous):
        ...
        if guidance_type == "uncond":
            ...
            if guidance_scale == 1. or unconditional_condition is None:
                ...
                # print(f'x : {x.size()}')
                x_in = torch.cat([x] * 2)
                # print(f'x_in : {x_in.size()}')
                t_in = torch.cat([t_continuous] * 2)
                # print(f't_in : {t_in.size()}')
                c_in = torch.cat([unconditional_condition, condition])
                # print(f'c_in : {c_in.size()}')
                # noise_uncond, noise = noise_pred_fn(x_in, t_in, cond=c_in).chunk(2)
                # print(f'noise_uncond : {noise_uncond.size()}')
                # print(f'noise        : {noise.size()}')
                model_out = noise_pred_fn(x_in, t_in, cond=c_in)
...
class DPM_Solver:
    ...
    def data_prediction_fn(self, x, t):
        ...
        noise, _ = torch.split(noise, 4, dim=1)
        noise = noise[:32, :, :, :]
        # noise = noise[32:, :, :, :]
        alpha_t, sigma_t = self.noise_schedule.marginal_alpha(t), self.noise_schedule.marginal_std(t)
        # print(f'x : {x.size()}')
        # print(f'sigma_t : {sigma_t.size()}')
        # print(f'solver inside noise : {noise.size()}')
